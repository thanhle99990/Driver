...
class _PhoneAuthVerifyState extends State<PhoneAuthVerify> {
  ...
  Widget build(BuildContext context) {
    ...
    /* return Scaffold(
      key: scaffoldKey,
      backgroundColor: Colors.white.withOpaqcab(0.95),
      body: SafeArea(
        child: Center(
      ...
      body: Column(
        ...
        children: <Widget>[
          ...
          /*Padding(
            ...
            child: Text('OTP: '+ verifyOTP, style: normalStylePrimary),
          ),*/
          UIHelper.verticalSpaceMedium,
          Expanded(
            child: Container(
              ...
              child: Column(
                children: [
                  MyH2(text: "Enter 6 digit verification code"),
                  UIHelper.verticalSpaceMedium,
                  Row(
          ...
          Padding(
            ...
            child:
                MyButton(caption: "Verify", fullsize: true, onPressed: signIn),
          )
        ],
      ),
    );
...
//          autofocus: key.contains("1") ? true : false,
          ...
          style: TextStyle(
              fontSize: 32.0, fontWeight: FontWeight.w600, color: PrimaryColor),
        ),
      );
...
//    _showSnackBar(phoneAuthDataProvider.message);
  ...
  onVerified() async {
    ...
    bool issignup = await _sharedPrefsHelper.isSignup;
    if (issignup) {
      UserObj newUser = Provider.of<UserObj>(context, listen: false);
      ResponseObj responseObj = await _authenticationService.signup(newUser);
      var loginSuccess = responseObj.code == 0;
      if (loginSuccess) {
        int idCustomer = responseObj.data;
        SharedPreferenceHelper _sharedPreferenceHelper =
            SharedPreferenceHelper();
        _sharedPreferenceHelper.setLogin(true);
        _sharedPreferenceHelper.setUserId(idCustomer);
        _authenticationService.getUserDetail();
...
//    _showSnackBar(phoneAuthDataProvider.message);
  ...
  onError() {
    _showSnackBar("Wrong verification code. Please, try again!");
//     _showSnackBar(
